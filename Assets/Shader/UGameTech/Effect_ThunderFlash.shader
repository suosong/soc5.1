Shader "UGameTech/Effect_ThunderFlash"
{
	Properties
	{
	...
	CGINCLUDE
		...
		fixed4 fragBloomWithColorMix( v2f_simple i ) : COLOR
		{
			...
			//> fixed4 colorTarget = { 0.35f, 0.15f, 0.75f, 1.0f };
			half colorDistance = Luminance(abs(color.rgb-colorTarget.rgb));
			color = lerp(color, colorTarget, (_Parameter.x*colorDistance));
			color += tex2D(_Bloom, i.uv.zw);
			*/

			fixed4 colorTarget = { 0.85f, 0.85f, 0.85f, 1.0f };
			half colorDistance = Luminance(color.rgb);
			color = lerp(color, colorTarget, (_Parameter.x*colorDistance));
			color += tex2D(_Bloom, i.uv.zw);
		...
		fixed4 fragMaxWithPain( v2f_withMaxCoords i ) : COLOR
		{
			...
			return saturate(color + half4(0.55,0.55,0.55,0) - ONE_MINUS_INTENSITY);
		} 
